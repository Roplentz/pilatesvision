...
describe("fisiovision integration contract", () => {
  it("libera exatamente os cinco exercícios validados", () => {
    expect([...FISIOVISION_ALLOWED_EXERCISES].sort()).toEqual(
      ["pilates-swan", "pilates-swimming", "pilates-single-leg-stretch", "pilates-teaser", "pilates-the-hundred"].sort(),
    );
  });

  it("tem rótulo humano para cada exercício", () => {
    for (const id of FISIOVISION_ALLOWED_EXERCISES) expect(FISIOVISION_EXERCISE_LABELS[id]).toBeTruthy();
  });
