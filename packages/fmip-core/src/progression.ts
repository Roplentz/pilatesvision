...
export function suggestProgression(
  ...
  const meanConfidence =
    recent.reduce((sum, session) => sum + session.captureConfidence, 0) / recent.length;
  const validRatios = recent.map((session) =>
    session.totalRepetitions > 0 ? session.validRepetitions / session.totalRepetitions : 0,
  );
  const meanValidRatio = validRatios.reduce((sum, value) => sum + value, 0) / validRatios.length;
  const painValues = recent
    .map((session) => session.pain)
    .filter((value): value is number => value !== undefined);
  const maxPain = painValues.length ? Math.max(...painValues) : undefined;
  const difficultyValues = recent
    .map((session) => session.perceivedDifficulty)
    .filter((value): value is number => value !== undefined);
  const maxDifficulty = difficultyValues.length ? Math.max(...difficultyValues) : undefined;

  if (meanConfidence < policy.minimumCaptureConfidence)
    reasons.push("capture_confidence_below_threshold");
  if (meanValidRatio < policy.minimumValidRepetitionRatio)
    reasons.push("valid_repetition_ratio_below_threshold");
  if (meanQuality < policy.minimumQuality) reasons.push("quality_below_progression_threshold");
  if (
    policy.maximumPainForProgression !== undefined &&
    maxPain !== undefined &&
    maxPain > policy.maximumPainForProgression
  )
    reasons.push("pain_above_progression_threshold");
  if (
    policy.maximumPerceivedDifficultyForProgression !== undefined &&
    maxDifficulty !== undefined &&
    maxDifficulty > policy.maximumPerceivedDifficultyForProgression
  )
    reasons.push("difficulty_above_progression_threshold");
