...
function scoreAgainstRange(
  ...
  range: NumericTargetRange | undefined,
): number | undefined {
...
export function assessSquatRepetitionV1(
  ...
  const trunkScore = scoreAgainstRange(
    observation.trunkInclinationP95Deg,
    protocol.targets.trunkInclinationP95Deg,
  );
  ...
  const dimensions: QualityDimensionInput[] = [
    {
      ...
      weight: protocol.weights.geometric,
    },
    {
      id: "temporal",
      score: scoreAgainstRange(observation.durationSeconds, protocol.targets.durationSeconds),
      confidence: observation.confidence,
      weight: protocol.weights.temporal,
    },
    {
      id: "symmetry",
      score: scoreAgainstRange(
        observation.bilateralDifferenceDeg,
        protocol.targets.bilateralDifferenceDeg,
      ),
      confidence: observation.confidence,
      weight: protocol.weights.symmetry,
    },
    {
      id: "smoothness",
      score: observation.smoothnessScore,
      confidence: observation.confidence,
      weight: protocol.weights.smoothness,
    },
    {
      id: "compensation",
      score: observation.compensationScore,
      confidence: observation.confidence,
      weight: protocol.weights.compensation,
    },
    {
      id: "completeness",
      score: observation.valid ? 100 : 0,
      confidence: observation.confidence,
      weight: protocol.weights.completeness,
    },
  ];
...
export function assessSquatV1(
  ...
  const repetitions = observations.map((observation) =>
    assessSquatRepetitionV1(observation, protocol),
  );
