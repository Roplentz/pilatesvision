...
export function nextSquatState(
  ...
  const canTransition = input.timestampSeconds - snapshot.stateEnteredAt >= config.minimumStateDurationSeconds;
  if (!canTransition) return snapshot;

  const movingDown = input.angularVelocityDegPerSecond >= config.movementVelocityThresholdDegPerSecond;
  const movingUp = input.angularVelocityDegPerSecond <= -config.movementVelocityThresholdDegPerSecond;
  let state = snapshot.state;
  let repetitionCount = snapshot.repetitionCount;

  if (state === "standing" && movingDown && input.kneeFlexionDeg > config.standingMaxFlexionDeg) state = "descending";
  else if (state === "descending" && input.kneeFlexionDeg >= config.bottomMinFlexionDeg) state = "bottom";
